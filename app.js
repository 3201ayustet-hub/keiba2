...
function openCompetitionCreate() {
  openModal(`
    <form id="competitionCreateForm" class="meeting-form unified-setting-form">
      <div class="setting-form-head">
      ...
      <section class="setting-section">
        <div class="setting-section-head"><span>MEETING</span><b>開催情報</b></div>
        <div class="setting-form-grid">
          <label class="wide">大会名
            <input name="name" required maxlength="100" placeholder="例：2027年 春競馬">
          </label>
          <label>開始日
      ...
      <section class="setting-section">
        <div class="setting-section-head"><span>NOTES</span><b>ルール・賞品・連絡事項</b></div>
        <label class="wide setting-textarea-label">
          <textarea name="topContent" maxlength="5000" placeholder="ルール、賞品、連絡事項を入力"></textarea>
        </label>
      </section>

      <p id="competitionCreateError" class="error" hidden></p>
      <div class="modal-actions setting-actions">
...
function openCompetitionEdit() {
  ...
  openModal(`
    <form id="competitionEditForm" class="meeting-form unified-setting-form">
      <div class="setting-form-head">
        <p class="eyebrow">MEETING SETTINGS</p>
        <h2>開催設定</h2>
      ...
      <section class="setting-section">
        ...
        <div class="setting-form-grid">
          <label class="wide">大会名
            <input name="name" required maxlength="100" value="${esc(competition.name)}">
          </label>
          <label>開始日
            <input name="startDate" type="date" value="${esc(competition.startDate)}">
          </label>
          <label>終了日
            <input name="endDate" type="date" value="${esc(competition.endDate)}">
          </label>
        </div>
      </section>
...
function openParticipantEditForm(participantId) {
  ...
  openModal(`
    <form id="participantEditForm" class="silk-editor-form">
      <input type="hidden" name="participantId" value="${esc(participant.id)}">
      <div class="silk-editor-head">
        <div><p class="eyebrow">JOCKEY SILKS</p><h2>参加者・勝負服設定</h2><small>変更は保存するまで確定されません。</small></div>
      </div>
      <label class="silk-name-label">参加者名<input name="name" required maxlength="50" value="${esc(participant.name)}"></label>
      <section class="silk-preview-stage">
        <div class="silk-preview-large" data-silk-preview-large>${silkPreviewMarkup(competition, participant, current, 'silk-preview-svg large-preview')}</div>
        <div class="silk-preview-copy"><strong>${esc(participant.name)}</strong><span>大きい表示</span></div>
        <div class="silk-preview-mini-row"><span data-silk-preview-mini>${silkPreviewMarkup(competition, participant, current, 'silk-preview-svg mini-preview')}</span><small>実際の一覧サイズ</small></div>
      </section>
      <div class="silk-control-grid">
        <label>胴の色<span class="silk-color-control"><input type="color" name="silksBodyColor" value="${esc(current.bodyColor)}"><code data-color-label="silksBodyColor">${esc(current.bodyColor.toUpperCase())}</code></span></label>
        <label>袖の色<span class="silk-color-control"><input type="color" name="silksSleeveColor" value="${esc(current.sleeveColor)}"><code data-color-label="silksSleeveColor">${esc(current.sleeveColor.toUpperCase())}</code></span></label>
        <label>柄の色<span class="silk-color-control"><input type="color" name="silksPatternColor" value="${esc(current.patternColor)}"><code data-color-label="silksPatternColor">${esc(current.patternColor.toUpperCase())}</code></span></label>
        <label>胴の柄<select name="silksBodyPattern">${SILK_PATTERNS.map(item => `<option value="${item.id}" ${item.id === current.bodyPattern ? 'selected' : ''}>${esc(item.name)}</option>`).join('')}</select></label>
        <label>袖の柄<select name="silksSleevePattern">${SILK_SLEEVE_PATTERNS.map(item => `<option value="${item.id}" ${item.id === current.sleevePattern ? 'selected' : ''}>${esc(item.name)}</option>`).join('')}</select></label>
      </div>
      <div class="silk-preset-block">
        <span>クイックカラー</span>
        ...
        <small>タップすると胴と袖を同じ色に設定します。そこから個別調整できます。</small>
      </div>
...
function refreshSilkEditorPreview() {
  ...
  ['silksBodyColor','silksSleeveColor','silksPatternColor'].forEach(name => {
    const label = form.querySelector(`[data-color-label="${name}"]`);
    if (label && form.elements[name]) label.textContent = String(form.elements[name].value).toUpperCase();
  });
  const nameNode = form.querySelector('.silk-preview-copy strong');
  ...
  app.innerHTML = `
    <section class="meeting-index season-index">
      <div class="season-index-head">
        ...
        <button class="button primary compact-create-meeting" type="button" data-action="new-competition">＋ 新大会</button>
      </div>
      ${state.competitions.length ? `<div class="meeting-index-list">${state.competitions
        ...
        }).join('')}</div>` : '<div class="empty">まだ大会がありません。「＋ 新大会」から作成してください。</div>'}
    </section>`;
}
    ...
    <div class="score-dashboard admin-dashboard">
      ...
      <section class="score-panel admin-score-panel meeting-lifecycle-panel">
        ...
        <div class="meeting-lifecycle-actions">
          ...
          <button type="button" class="meeting-admin-link" data-action="new-competition">
            <span><strong>次大会を作成</strong><small>例：2027年 春競馬</small></span><b>＋</b>
          </button>
        </div>
      </section>
