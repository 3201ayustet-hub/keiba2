...
function finishBoardPanel(competition, summary) {
  if (!summary.maxPayoutRanking.length) return '<div class="score-empty">参加者を登録すると着順掲示板が表示されます。</div>';
  const rows = summary.maxPayoutRanking.slice(0, 8);
  const lamps = rows.map(row => `<span class="lane-lamp">${romanRank(row.rank)}</span>`).join('');
  return `<div class="max-board">
    <aside class="max-board-rail" aria-hidden="true">
      <strong>HBB</strong>
      <div>${lamps}</div>
    </aside>
    <div class="max-board-main">
      <div class="score-panel-head"><span>MAX PAYOUT BOARD</span><b>${competition.status === 'closed' ? '確定' : '暫定'}</b></div>
      <div class="max-board-title"><span>最高払戻</span><strong>${esc(competition.name)}</strong></div>
      <div class="max-board-cols"><span>着</span><span>PLAYER / RACE</span><span>払戻金</span></div>
      <div class="max-board-rows">
        ${rows.map(row => {
          const p = competition.participants.find(x => x.id === row.participantId);
          return `<button class="max-board-row ${row.rank === 1 ? 'is-leader' : ''}" type="button" data-action="view-participant" data-participant-id="${esc(row.participantId)}">
            <span class="board-place">${romanRank(row.rank)}</span>
            <span class="board-player">${p ? silkMark(competition,p,'silk-icon score-silk') : ''}<span><strong>${esc(row.name)}</strong><small>${esc(row.maxPayoutRace || '—')}</small></span></span>
            <b>¥${Number(row.maxPayout || 0).toLocaleString('ja-JP')}</b>
          </button>`;
        }).join('')}
      </div>
      <div class="max-board-foot">
        <span>判定 <b>1レース最高払戻額</b></span>
        <button type="button" data-action="view-rank-history">順位推移を見る ›</button>
      </div>
    </div>
  </div>`;
}

function returnPanel(competition, summary) {
  const top = summary.recoveryRanking[0];
  if (!top) return `<section class="score-panel return-panel"><div class="score-panel-head"><span>RETURN RATE</span></div><div class="score-empty compact">参加者未登録</div></section>`;
  return `<section class="score-panel return-panel">
    <div class="score-panel-head"><span>TOP RETURN</span><small>回収率</small></div>
    <button class="return-hero" type="button" data-action="view-participant" data-participant-id="${esc(top.participantId)}">
      <strong>${pct(top.recoveryRate)}</strong><span>${esc(top.name)}</span><small>${top.profit >= 0 ? '+' : ''}${yen(top.profit)}</small>
    </button>
    <div class="return-mini-list">
      ${summary.recoveryRanking.slice(0,4).map((row,i)=>`<button type="button" data-action="view-participant" data-participant-id="${esc(row.participantId)}"><span>${i+1}</span><b>${esc(row.name)}</b><em>${pct(row.recoveryRate)}</em></button>`).join('')}
    </div>
  </section>`;
}

function leaderPanel(competition, summary) {
  const champ = summary.maxPayoutRanking[0];
  if (!champ) return '';
  const p = competition.participants.find(x=>x.id===champ.participantId);
  return `<button class="score-panel leader-panel" type="button" data-action="view-participant" data-participant-id="${esc(champ.participantId)}">
    <div class="score-panel-head"><span>TOP PLAYER</span><small>一撃王</small></div>
    <div class="leader-body">${p ? silkMark(competition,p,'silk-icon hero-silk') : ''}<span><strong>${esc(champ.name)}</strong><small>${esc(champ.maxPayoutRace || '—')}</small></span><b>¥${Number(champ.maxPayout||0).toLocaleString('ja-JP')}</b></div>
  </button>`;
}

function nextRacePanel(competition) {
  const race = nextRaceFor(competition);
  if (!race) return `<section class="score-panel next-panel"><div class="score-panel-head"><span>NEXT RACE</span></div><div class="score-empty compact">レース未登録</div></section>`;
  return `<button class="score-panel next-panel" type="button" data-action="view-race" data-race-id="${esc(race.id)}">
    <div class="score-panel-head"><span>NEXT RACE</span><small>${race.gradeType === 'G1' ? 'G1' : '非G1'}</small></div>
    <div class="next-body"><span>${esc(raceDateDisplay(race.raceDateTime))}</span><strong>${esc(race.name)}</strong><small>${esc(race.racecourse || '競馬場未設定')}</small><b>›</b></div>
  </button>`;
}
...
function raceProgramTable(competition) {
  ...
  return `<div class="race-board">
    <div class="race-board-head"><span>日付</span><span>場</span><span>格</span><span>レース名</span><span>入力</span><span></span></div>
    ${races.map(race => {
      const completed = race.entries.filter(e=>e.status !== 'undecided').length;
      return `<button class="race-board-row" type="button" data-action="view-race" data-race-id="${esc(race.id)}">
        <span class="mono">${esc(raceDateDisplay(race.raceDateTime))}</span>
        <span>${esc(race.racecourse || '—')}</span>
        <span class="race-class ${race.gradeType === 'G1' ? 'g1' : 'nong1'}">${race.gradeType === 'G1' ? 'G1' : '非G1'}</span>
        <strong>${esc(race.name)}</strong>
        <span class="mono">${completed}/${competition.participants.length}</span>
        <b>›</b>
      </button>`;
    }).join('')}
  </div>`;
...
function playersScorePanel(competition, summary) {
  return `<section class="score-panel players-score-panel" id="dashboard-players">
    <div class="score-panel-head"><span>PLAYERS</span><small>回収率 / 権利</small></div>
    <div class="players-score-list">${summary.recoveryRanking.map((row,index)=>{
      const p = competition.participants.find(x=>x.id===row.participantId);
      const rights = rightsForParticipant(competition,row.participantId);
      return `<button type="button" data-action="view-participant" data-participant-id="${esc(row.participantId)}">
        <span class="player-rank">${index+1}</span>
        ${p ? silkMark(competition,p,'silk-icon score-silk') : ''}
        <span class="player-copy"><strong>${esc(row.name)}</strong><small>5千×${rights.bonusRemaining} ・ 非G1×${rights.nonG1Remaining}</small></span>
        <span class="player-rate"><b>${pct(row.recoveryRate)}</b><small>${row.profit>=0?'+':''}${yen(row.profit)}</small></span>
      </button>`;
    }).join('') || '<div class="score-empty compact">参加者未登録</div>'}</div>
  </section>`;
}

function recentResultPanel(competition) {
  const race = latestCompletedRace(competition);
  if (!race) return `<section class="score-panel recent-panel"><div class="score-panel-head"><span>RACE RESULT</span><small>直近レース</small></div><div class="score-empty compact">結果未入力</div></section>`;
  const rows = race.entries.filter(e=>e.status==='join').map(e=>{
    const p=competition.participants.find(x=>x.id===e.participantId);
    return { entry:e, participant:p, payout:Number(e.payoutAmount||0) };
  }).sort((a,b)=>b.payout-a.payout);
  return `<section class="score-panel recent-panel">
    <button class="recent-race-head" type="button" data-action="view-race" data-race-id="${esc(race.id)}"><span>${esc(raceDateDisplay(race.raceDateTime))}</span><strong>${esc(race.name)}</strong><small>${race.gradeType==='G1'?'G1':'非G1'} ${esc(race.racecourse||'')}</small></button>
    <div class="recent-result-list">${rows.slice(0,8).map((row,i)=>`<div><span class="recent-place">${romanRank(i+1)}</span>${row.participant?silkMark(competition,row.participant,'silk-icon mini-silk'):''}<b>${esc(row.participant?.name||'—')}</b><strong>¥${row.payout.toLocaleString('ja-JP')}</strong></div>`).join('') || '<div class="score-empty compact">参加者なし</div>'}</div>
  </section>`;
}

function ticketPreviewPanel(competition, summary) {
  const champ = summary.maxPayoutRanking[0];
  if (!champ) return `<section class="score-panel ticket-preview-panel"><div class="score-panel-head"><span>BET TICKET</span><small>デジタル馬券</small></div><div class="score-empty compact">払戻記録なし</div></section>`;
  const record = maxRecordFor(competition,champ.participantId);
  const p = competition.participants.find(x=>x.id===champ.participantId);
  if (!record || !p) return '';
  const rate = record.entry.betAmount ? Number(record.entry.payoutAmount||0)/Number(record.entry.betAmount)*100 : null;
  return `<section class="score-panel ticket-preview-panel">
    <div class="score-panel-head"><span>BET TICKET</span><small>最高払戻記録</small></div>
    <button class="ticket-preview" type="button" data-action="view-ticket" data-race-id="${esc(record.race.id)}" data-participant-id="${esc(p.id)}">
      <div class="ticket-preview-top"><small>HBB DIGITAL BET TICKET</small><b>${record.race.gradeType==='G1'?'G1':'SPECIAL'}</b></div>
      <div class="ticket-preview-title"><strong>${esc(record.race.name)}</strong><span>${esc(raceDateDisplay(record.race.raceDateTime))}</span></div>
      <div class="ticket-preview-player">${silkMark(competition,p,'silk-icon mini-silk')}<b>${esc(p.name)}</b></div>
      <div class="ticket-preview-money"><span>BET <b>¥${Number(record.entry.betAmount||0).toLocaleString('ja-JP')}</b></span><span>PAYOUT <b>¥${Number(record.entry.payoutAmount||0).toLocaleString('ja-JP')}</b></span></div>
      <div class="ticket-preview-barcode"></div>
      <div class="ticket-preview-foot"><span>RETURN ${pct(rate)}</span><span>OPEN ›</span></div>
    </button>
  </section>`;
}
  ...
  app.innerHTML = `
    <div class="score-dashboard">
      <div class="score-top">
        ...
        <div class="score-top-actions">
          <span>${competition.participants.length} PLAYERS / ${competition.races.length} RACES</span>
          <button class="score-menu" type="button" data-action="open-admin" aria-label="大会管理">☰</button>
        </div>
      </div>

      <div class="score-hero-grid" id="dashboard-board">
        ${finishBoardPanel(competition, summary)}
        <aside class="score-side-stack">
          ${returnPanel(competition, summary)}
          ${leaderPanel(competition, summary)}
          ${nextRacePanel(competition)}
        </aside>
      </div>
      ...
      <div class="score-lower-grid">
        ${playersScorePanel(competition, summary)}
        ${recentResultPanel(competition)}
        ${ticketPreviewPanel(competition, summary)}
      </div>

      ${competition.topContent ? `<section class="score-panel rules-score-panel" id="dashboard-rules"><div class="score-panel-head"><span>MEETING NOTES</span><small>開催要項・賞品</small></div><div class="rules-score-body">${esc(competition.topContent)}</div></section>` : ''}

      <nav class="score-bottom-nav" aria-label="画面内ナビゲーション">
        <button type="button" data-scroll-target="#dashboard-board"><b>⌂</b><span>TOP</span></button>
        <button type="button" data-scroll-target="#dashboard-races"><b>≡</b><span>RACES</span></button>
        <button type="button" data-scroll-target="#dashboard-players"><b>♙</b><span>PLAYERS</span></button>
        <button type="button" data-action="view-rank-history"><b>↗</b><span>RANK</span></button>
        <button type="button" data-action="open-admin"><b>⚙</b><span>管理</span></button>
      </nav>
    </div>`;
}
...
function renderAdmin(competition) {
  const summary = summaryFor(competition);
  app.innerHTML = `
    <div class="top-nav"><button class="button ghost nav-button" type="button" data-action="back-dashboard">← ボードへ戻る</button></div>
    <section class="admin-hero section"><p class="eyebrow">MEETING CONTROL</p><h2>大会管理</h2><p>${esc(competition.name)}</p></section>
    <section class="section admin-block"><div class="section-head compact"><div><h2>勝負設定</h2><p class="muted">勝負名・期間・ルール・賞品</p></div><button class="button secondary" type="button" data-action="edit-competition">編集</button></div></section>
    <section class="section admin-block"><div class="section-head compact action-head"><div><h2>参加者管理</h2><p class="muted">名前・勝負服・削除</p></div><button class="round-add" type="button" data-action="add-participant" ${competition.participants.length>=8?'disabled':''}>＋</button></div>
      <div class="admin-list">${competition.participants.map(p => adminParticipantRow(competition, summary, p)).join('') || '<div class="empty">参加者がいません。</div>'}</div></section>
    <section class="section admin-block"><div class="section-head compact"><div><h2>レース管理</h2><p class="muted">レースの追加はボード、編集・削除は各レース詳細から行います。</p></div></div></section>
...
function adminParticipantRow(competition, summary, participant) {
  ...
  return `<div class="admin-row" data-participant-id="${esc(participant.id)}"><span class="admin-silk">${silkMark(competition, participant,'silk-icon')}</span><span><strong>${esc(participant.name)}</strong><small>回収率 ${pct(row?.recoveryRate)}</small></span><span class="admin-actions"><button class="button ghost" type="button" data-action="edit-participant">編集</button><button class="button danger" type="button" data-action="delete-participant" data-participant-id="${esc(participant.id)}">削除</button></span></div>`;
}
...
function openParticipantDetail(participantId) {
  ...
  openModal(`<div class="form-guide-head">${silkMark(competition,participant,'silk-icon xl')}<div><p class="eyebrow">FORM GUIDE</p><h2>${esc(participant.name)}</h2><small>個人成績 / 馬柱</small></div><button class="button ghost" type="button" data-close-modal>閉じる</button></div>
