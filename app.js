...
function renderList() {
  app.innerHTML = `
    <section class="meeting-index">
      <div class="meeting-index-head">
        <div class="meeting-title-mark"><i></i><b>HBB</b><i></i></div>
        <div><p class="eyebrow">RACE MEETINGS</p><h2>勝負一覧</h2><small>Supabase共有</small></div>
      </div>
      ${state.competitions.length ? `<div class="meeting-index-list">${state.competitions.map(competition => `
        <button type="button" class="meeting-index-row" data-open-competition="${esc(competition.id)}">
          ...
          <span class="meeting-index-status">開催中</span><b>›</b>
        </button>`).join('')}</div>` : '<div class="empty">まだ勝負がありません。「＋ 新しい勝負」から作成してください。</div>'}
    </section>`;
}
...
function renderCompetition(competition) {
  ...
  app.innerHTML = `
    <div class="score-dashboard v41-dashboard">
      <header class="meeting-header race-meeting-title">
        <div class="meeting-title-mark" aria-hidden="true"><i></i><b>HBB</b><i></i></div>
...
function renderAdmin(competition) {
  ...
  app.innerHTML = `
    <div class="score-dashboard admin-dashboard">
      <header class="meeting-header race-meeting-title admin-meeting-header">
        <div class="meeting-title-mark" aria-hidden="true"><i></i><b>HBB</b><i></i></div>
        <div class="meeting-title-copy">
