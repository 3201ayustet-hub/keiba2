...
function renderCompetition(competition) {
  ...
  const statusLabel = competition.status === 'closed' ? '確定' : '暫定';

  app.innerHTML = `
    <div class="top-nav">
      <button class="button ghost nav-button" type="button" data-action="back">← 勝負一覧</button>
      <button class="icon-admin" type="button" data-action="open-admin" aria-label="大会管理">•••</button>
    </div>

    <section class="meeting-board section">
      <div><p class="micro-label">RACE MEETING</p><h2>${esc(competition.name)}</h2><p>${esc(shortPeriodDate(competition.startDate))} — ${esc(shortPeriodDate(competition.endDate))}</p></div>
      <dl><div><dt>PLAYERS</dt><dd>${competition.participants.length}</dd></div><div><dt>RACES</dt><dd>${competition.races.length}</dd></div></dl>
    </section>

    <section class="section result-board-section">
      <div class="section-head compact"><div><p class="eyebrow gold-text">RESULT BOARD</p><h2>最大払戻額ランキング</h2></div><span class="board-status">${statusLabel}</span></div>
      ${champion ? championBoard(competition, champion, topThree) : '<div class="empty">参加者を登録するとランキングが表示されます。</div>'}
      ${competition.races.length && competition.participants.length ? '<button class="history-entry" type="button" data-action="view-rank-history"><span>順位推移</span><small>レースごとの隊列変化</small><b>›</b></button>' : ''}
    </section>

    <section class="section ranking-section">
      <div class="section-head compact"><div><p class="eyebrow">PERFORMANCE</p><h2>総合回収率</h2></div></div>
      ${recoveryList(competition, summary.recoveryRanking)}
    </section>

    <section class="section race-program-section">
      <div class="section-head compact action-head"><div><p class="eyebrow">RACE PROGRAM</p><h2>登録レース</h2></div><button class="round-add" aria-label="レースを追加" type="button" data-action="add-race" ${competition.participants.length ? '' : 'disabled'}>＋</button></div>
      ${races.length ? `<div class="race-program">${races.map(race => raceListItem(competition, race)).join('')}</div>` : '<div class="empty">参加者を登録後、レースを追加してください。</div>'}
    </section>

    <section class="section players-section">
      <div class="section-head compact"><div><p class="eyebrow">RUNNERS</p><h2>参加者</h2></div></div>
      ${competition.participants.length ? `<div class="participant-list">${competition.participants.map(participant => participantCard(competition, summary, participant)).join('')}</div>` : '<div class="empty">参加者は大会管理から登録できます。</div>'}
    </section>

    ${competition.topContent ? `<section class="section meeting-notes"><div class="section-head compact"><div><p class="eyebrow gold-text">MEETING NOTES</p><h2>開催要項・賞品</h2></div></div><div class="meeting-notes-body">${esc(competition.topContent)}</div></section>` : ''}`;
}
